# Sideload the ScreenHarbor iOS client

ScreenHarbor publishes a device-only, unsigned IPA for iOS 18 and later. It uses
the same authenticated transport and `_screenharbor._tcp` discovery service as
the current ScreenHarbor Host.

## Install

1. Download the `ScreenHarbor-iOS-…-unsigned.ipa` file and its `.sha256` file
   from the project release or website.
2. Verify the checksum:

   ```bash
   shasum -a 256 -c ScreenHarbor-iOS-*.ipa.sha256
   ```
...
5. On first launch, allow Local Network access so ScreenHarbor can discover
   ScreenHarbor Host.
...
- Client: ScreenHarbor for iOS 1.0.3 or later
- Minimum OS: iOS/iPadOS 18
- Host: ScreenHarbor Host 1.0.2 or later
...
still requires explicit approval in ScreenHarbor Host.
